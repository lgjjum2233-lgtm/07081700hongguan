...
DATE_FIRST_COL = 6
DATE_LAST_COL = 534  # TN, matching the front-sheet formulas.
MACRO_SHEET_INDEX = 4
CHINA_MACRO_SHEET_INDEX = 2
...
INDICATORS = [
    Indicator("中国:金融机构:新增人民币存款:财政存款:累计值", "M0048264", "月度", "亿元"),
    Indicator("中国:社会消费品零售总额:当月值", "M0001427", "月度", "亿元"),
    Indicator("中国:GDP:不变价:当季同比", "M0039354", "季度", "%"),
]
...
def find_raw_end(macro) -> int:
    ...
    if not match:
        raise RuntimeError("Cannot parse WSD raw-row range from macro!F2")
...
def front_rows_by_code(ws) -> dict[str, int]:
    rows: dict[str, int] = {}
    for row in range(1, ws.max_row + 1):
...
def macro_rows_by_code(macro) -> dict[str, int]:
    rows: dict[str, int] = {}
...
def main() -> None:
    ...
    front = wb.worksheets[CHINA_MACRO_SHEET_INDEX]
    macro_title = macro.title

    macro_max_col = max(macro.max_column, DATE_LAST_COL)
    old_raw_end = find_raw_end(macro)
    existing_macro = macro_rows_by_code(macro)
    existing_front = front_rows_by_code(front)
    items_to_add = [item for item in INDICATORS if norm(item.code) not in existing_macro]
    front_to_add = [item for item in INDICATORS if norm(item.code) not in existing_front]

    if len(items_to_add) != len(front_to_add):
        missing = {norm(i.code) for i in items_to_add} ^ {norm(i.code) for i in front_to_add}
        raise RuntimeError(f"Macro/front duplicate state is inconsistent: {sorted(missing)}")
    ...
    if items_to_add:
        ...
        for offset, item in enumerate(items_to_add):
        ...
        for row in range(old_max_row + 1, old_max_row + add_count + 1):
            pass

    new_raw_end = old_raw_end + len(items_to_add)
    ...
    verify_macro = verify.worksheets[MACRO_SHEET_INDEX]
    verify_front = verify.worksheets[CHINA_MACRO_SHEET_INDEX]
    errors: list[str] = []

    f2_after = formula_text(verify_macro["F2"].value)
    if f"E2:E{new_raw_end}" not in f2_after or f"rows={new_raw_end - 1}" not in f2_after:
        errors.append("macro!F2 WSD range was not updated")

    verify_macro_rows = macro_rows_by_code(verify_macro)
    verify_front_rows = front_rows_by_code(verify_front)
    for item in INDICATORS:
        macro_row = verify_macro_rows.get(norm(item.code))
        front_row = verify_front_rows.get(norm(item.code))
        if macro_row is None:
            errors.append(f"missing macro row: {item.code}")
        elif macro_row > new_raw_end:
            errors.append(f"macro row outside raw WSD range: {item.code} row={macro_row}")
        if front_row is None:
            errors.append(f"missing front row: {item.code}")
        else:
            chart = chart_by_row(verify_front, front_row)
            if chart is None:
                errors.append(f"missing front chart: {item.code} row={front_row}")
            else:
                expected_vals = f"'{verify_macro.title}'!$F${macro_row}:$TN${macro_row}"
                found_vals = []
                for series in chart.series:
                    try:
                        found_vals.append(series.val.numRef.f)
                    except Exception:
                        pass
                if expected_vals not in found_vals:
                    errors.append(f"chart source mismatch: {item.code} expected={expected_vals} found={found_vals}")

    report = {
        "input": str(input_path),
        "output": str(output_path),
        "macro_sheet": verify_macro.title,
        "front_sheet": verify_front.title,
        ...
        "front_max_row": verify_front.max_row,
        "front_charts": len(verify_front._charts),
        "errors": errors,
    }
